    ...
    const incomeData = await household_model.findByPk(householdId,{
        ...
        include:{
            ...
            include:{
                ...
                attributes:['id'],
                include:[non_agi_model]
            }
        }
    })

    //รวมยอดเงินถ้าสร้างวันเดียวกันเป็น 1 value , year
    const incomeMap = new Map(); //สร้าง {createdAt: จำนวนวเงิน}

    incomeData.Form.Financialcapital.NonAGIincomes.forEach(item => {
      const createdAt = new Date(item.createdAt).toISOString().split('T')[0]; //  (YYYY-MM-DD)
      //เจอวันที่ซ้ำ + ทบไป
      if (incomeMap.has(createdAt)) {
        incomeMap.set(createdAt, incomeMap.get(createdAt) + item.amount_per_year);
      } 
      //ไม่เจอซ้ำสร้างใหม่
      else {
        incomeMap.set(createdAt, item.amount_per_year); //key:value
      }
    });

    // เพิ่มค่าที่รวมแล้วลงใน value และ year
    incomeMap.forEach((amount, date) => {
      value.push(amount);
      year.push(new Date(date).getFullYear());
    });

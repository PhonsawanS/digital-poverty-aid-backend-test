...
//validate type
const { formSchema } = require('../validators/FormInsertAll/form.insert.all.validator')
...
const create = async(req,res)=>{
    ...
    try{
        ...
        const result = await db.sequelize.transaction(async(t)=>{
            ...
            const Informant = await Informant_model.create(
                {
                    ...
                    live_but_has_no_name_in_fam: value.Informant.live_but_has_no_name_in_fam,
                    form_id: Form.id,
                },
                { transaction: t }
            )
            ...
            const MemberHouseHoldArr = await Promise.all(MemberHouseHoldPromise)
            ...
            const PhysicalCapital = await PhysicalCapital_model.create(
                {
                    ...
                    use_tech_get_benefit_gov: value.PhysicalCapital.use_tech_get_benefit_gov,
            ...
            const UtilityWater = await UtilityWater_model.create(
                {
                    ...
                    water_other_sources: value.PhysicalCapital.UtilityWater.water_other_sources,
            ...
            const UrbanArea = await UrbanArea_model.create(
                {
                    is_use_area_to_work: value.PhysicalCapital.UrbanArea.is_use_area_to_work,
                    has_prolem_in_area: value.PhysicalCapital.UrbanArea.has_prolem_in_area,
                    phy_capital_id: PhysicalCapital.id
                },
                {transaction:t}
            )
            ...
            const AgriculturalincomePromise = value.Financialcapital.Agriculturalincome.map(async(data)=>{
            ...
            const AgriculturalincomeArr = await Promise.all(AgriculturalincomePromise)
            

            const NonAGIincomePromise = value.Financialcapital.NonAGIincome.map(async(data)=>{
            ...
            const DebtPromise = value.Financialcapital.Debt.map(async(data)=>{
                ...
                const CreditsourcesPromise = value.Financialcapital.Creditsources.map(async(data)=>{
            ...
            const OccupationalpropertyPromise = value.Financialcapital.Occupationalproperty.map(async(data)=>{
            ...
            const OccupationalpropertyArr = await Promise.all(OccupationalpropertyPromise)

            const HouseholdexpensesPromise = value.Financialcapital.Householdexpenses.map(async(data)=>{
            ...
            const PBresourceforincomePromise = value.Naturalcapital.PBresourceforincome.map(async(data)=>{
            ...
            const PBresourceforincomeArr = await Promise.all(PBresourceforincomePromise)

            const PBresourceforlivePromise = value.Naturalcapital.PBresourceforlive.map(async(data)=>{
            ...
            const FarmlandindisasterareasPromise = value.Naturalcapital.Farmlandindisasterareas.map(async(data)=>{
            ...
            const FarmlandindisasterareasArr = await Promise.all(FarmlandindisasterareasPromise)

            const HouseInDisasterAreasPromise = value.Naturalcapital.HouseInDisasterAreas.map(async(data)=>{
            ...
            const HouseInDisasterAreasArr = await Promise.all(HouseInDisasterAreasPromise)
            ...
            const ActivitygrouptypePromise = value.Socialcapital.Activitygrouptype.map(async(data)=>{
            ...
            const ActivitytypePromise = value.Socialcapital.Activitytype.map(async(data)=>{
